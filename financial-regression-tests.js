...
assertFileIncludes(appCode, "let expandedClientIds = new Set();", "Clientes: las ampliaciones deben controlar su estado abierto/cerrado por cliente.");
...
assertFileIncludes(stylesCode, ".client-extension-panel {\n  grid-column: 1 / -1;\n  display: grid;", "Clientes: las ampliaciones deben quedar pegadas al cliente principal.");
assertFileIncludes(stylesCode, "grid-template-rows: 0fr;", "Clientes: las ampliaciones deben iniciar colapsadas visualmente.");
assertFileIncludes(stylesCode, ".client-extension-panel.is-expanded", "Clientes: debe existir estado visual desplegado para ampliaciones.");
assertFileIncludes(stylesCode, ".extension-toggle", "Clientes: el chevron de ampliaciones debe tener estilo propio.");
...
assertFileIncludes(htmlCode, 'data-open-privacy', "Privacidad: debe existir acceso visible a Politica de Privacidad.");
assertFileIncludes(htmlCode, 'id="privacyDialog"', "Privacidad: debe existir modal independiente de Politica de Privacidad.");
assertFileIncludes(htmlCode, 'id="privacyRequestDialog"', "Privacidad: debe existir formulario de derechos ARCO.");
assertFileIncludes(htmlCode, 'id="accountDeletionDialog"', "Cuenta: debe existir solicitud de eliminacion separada.");
assertFileIncludes(htmlCode, 'id="adminClaimsList"', "Admin Reclamos: debe existir lista administrativa de reclamos.");
assertFileIncludes(htmlCode, 'id="adminPrivacyList"', "Admin Privacidad: debe existir lista de solicitudes ARCO.");
assertFileIncludes(htmlCode, 'id="adminDeletionList"', "Admin Privacidad: debe existir lista de solicitudes de eliminacion.");
...
assertFileIncludes(appCode, "claimSubmissionInProgress", "Libro de Reclamaciones: debe evitar doble clic al registrar hojas.");
assertFileIncludes(appCode, 'apiFetch("/api/reclamaciones"', "Libro de Reclamaciones: el frontend debe registrar la hoja via Worker.");
assertFileIncludes(appCode, "calculateClaimDueDate", "Libro de Reclamaciones: debe centralizar fecha limite de 15 dias habiles.");
assertFileIncludes(appCode, "PRIVACY_REQUEST_RESPONSE_DAYS", "Privacidad: plazos ARCO deben estar separados del Libro de Reclamaciones.");
assertFileIncludes(appCode, "/api/admin/reclamaciones/", "Admin Reclamos: respuestas deben pasar por Worker.");
...
assertFileIncludes(workerCode, "/api/admin/legal", "Admin Legal: Worker debe listar expedientes con autorizacion real.");
assertFileIncludes(workerCode, "assertAdminUser", "Admin Legal: Worker debe verificar administrador en servidor.");
assertFileIncludes(workerCode, "queueTransactionalEmail", "Correos: debe existir abstraccion de email sin fingir envio.");
assertFileIncludes(workerCode, "pending_configuration", "Correos: sin proveedor real debe quedar pendiente de configuracion.");
assertFileIncludes(workerCode, "claim_response:", "Correos: respuesta de reclamo debe usar evento idempotente.");
...
assertFileIncludes(schemaCode, "create table if not exists privacy_requests", "Privacidad: esquema base debe crear tabla de solicitudes ARCO.");
assertFileIncludes(schemaCode, "create table if not exists account_deletion_requests", "Cuenta: esquema base debe crear solicitudes de eliminacion.");
assertFileIncludes(schemaCode, "create table if not exists email_outbox", "Correos: esquema base debe tener outbox idempotente.");
assertFileIncludes(schemaCode, "document_type text", "Clientes: esquema base debe permitir documento opcional.");
assertFileIncludes(schemaCode, "privacy_policy_version", "Consentimiento: perfil debe conservar version de Politica de Privacidad.");
assertFileIncludes(readText("supabase-legal-privacy.sql"), "create table if not exists claim_book_events", "Migracion legal: debe crear trazabilidad de reclamos.");
assertFileIncludes(readText("supabase-legal-privacy.sql"), "revoke all on claim_book_entries from anon, authenticated", "RLS legal: no debe existir SELECT publico sobre reclamos.");
assertFileIncludes(readText("supabase-legal-privacy.sql"), "email_outbox", "Migracion legal: debe preparar cola/outbox de correos.");
assertFileIncludes(readText("PRIVACY-POLICY.md"), "ERMIF no utiliza estos datos para publicidad ni marketing", "Privacidad: no debe documentar marketing como finalidad.");
assertFileIncludes(readText("DATA-TRANSFER-REVIEW.md"), "Region conocida", "Transferencias: debe documentar region de proveedores como conocida/desconocida.");
